...
* `current_clause`: A dictionary containing `id`, `text`, `category`, and `is_ambiguous`.
* `clauses_reviewed`, `total_clauses`, `cumulative_reward`, `flags_raised`, `false_approvals`, `escalations`, `done`.

ALL rewards are floating-point values strictly bound within `[-1.0, 1.0]`.
...
1. **`nda_review` (Difficulty: Easy)**: Hard limit of 10 sequential clause steps. Focuses on confidentiality and liability risks.
2. **`saas_review` (Difficulty: Medium)**: Hard limit of 15 sequential clause steps. Covers IP ownership, data privacy, and SLA penalties.
3. **`ma_review` (Difficulty: Hard)**: Hard limit of 20 sequential clause steps. Tests indemnity chains, cross-jurisdiction risk, and ambiguous clauses.
...
The evaluator (`inference.py`) produces EXACTLY four line formats continuously to `stdout`, omitting JSON multi-liners and debugging footprints for total reproducibility:
...
* `[END] success=<true|false> steps=<n> score=<score> rewards=<r1,r2,...,rn>`: Emitted once upon episode termination.
* `[SUMMARY] tasks=<n> avg_score=<score> scores=<s1,s2,s3>`: Emitted once after all tasks complete.
