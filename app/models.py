...
class Clause(BaseModel):
    ...
    risk_level: str = Field(default="low", description="Planted risk level: low / medium / high")
    is_ambiguous: bool = Field(default=False, description="True if clause is intentionally ambiguous")
...
class ActionParams(BaseModel):
    action: str = Field(
        ...,
        description="Agent decision: approve | flag | redline | escalate"
    )
    reason: str = Field(
        default="",
    ...
    suggested_edit: Optional[str] = Field(
        default=None,
        description="Rewritten clause text when action is redline, else null"
    )
...
class State(BaseModel):
    ...
    clauses_reviewed: int = Field(default=0, description="Number of clauses processed so far")
    total_clauses: int = Field(default=0, description="Total clauses in this contract")
    cumulative_reward: float = Field(default=0.0, description="Running reward total for this episode")
    flags_raised: list[str] = Field(default_factory=list, description="IDs of clauses flagged by agent")
