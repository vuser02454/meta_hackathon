...
# Fallback reasons that should NOT trigger the bonus reward
FALLBACK_REASONS = {
    "no reason provided",
...
- Missing a risk (false negative) = hurts your risk detection score
- Over-flagging safe clauses = lowers your efficiency score
...
6. Write a one-sentence reason explaining your decision clearly.
...
def apply_risk_override(action: str, clause_text: str) -> str:
    """Force flag if agent tries to approve a clause with known high-risk keywords."""
    if action == "approve":
...
def call_llm(client: OpenAI, prompt: str, task_id: str, retry: bool = True) -> dict:
    ...
    try:
        ...
        return {
            "action": "flag",
            "reason": f"api error ({type(e).__name__}) fallback",
            "suggested_edit": None,
        }
...
def run_task(env: LegalContractEnv, task_id: str, client: OpenAI) -> tuple:
    ...
    while not state.done:
        ...
        if time.time() - task_start > MAX_TASK_SECONDS:
            log_step(state.clauses_reviewed, "timeout", 0.0, True, "timeout")
            break

        # ── Guard: memory limit ───────────────────────────────────────────
        if get_memory_mb() > MAX_MEMORY_MB:
            log_step(state.clauses_reviewed, "timeout", 0.0, True, "memory_limit")
            break
        ...
        # Safety net: never approve known high-risk keywords
...
def main():
    ...
    # FIX: Each task isolated — one crash does not kill other tasks
    for task in tasks:
        try:
    ...
    # FIX: Summary log after all tasks complete
